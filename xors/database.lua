...
function Database:has(table_name, column_name)
    if table_name then
        ...
        if column_name then
            ...
            for n, c in ipairs(columns) do
                if c.name == tostring(column_name) then
                    column_exists = true
                    break
                end
            end
...
function Database:rename(table_name, ...)
    ...
    if type(arguments[2]) == "string" then
        ...
        if column_name ~= new_column_name and self:has(table_name, column_name) then
            print("done renaming")
        ...
        if table_name ~= new_table_name and self:has(table_name) then
